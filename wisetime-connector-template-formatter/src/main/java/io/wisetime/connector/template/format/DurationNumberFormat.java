...
public class DurationNumberFormat extends TemplateNumberFormat {
  ...
  public String formatToPlainText(TemplateNumberModel numberModel)
    ...
    try {
      return secondsToHumanString((int) Math.ceil(number.doubleValue()));
    } catch (ArithmeticException e) {
      throw new UnformattableValueException(number + " doesn't fit into an int");
    }
  }

  private String secondsToHumanString(int seconds) {
    return PERIOD_FORMATTER.print(Period.seconds(seconds).normalizedStandard());
  }
