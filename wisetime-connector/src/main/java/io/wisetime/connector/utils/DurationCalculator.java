...
public class DurationCalculator {
  ...
  private final Function<Double, Double> applyExperienceWeighting = duration -> {
    if (useExperienceWeighting && timeGroup.getUser() != null) {
      Integer weightingPercent = timeGroup.getUser().getExperienceWeightingPercent();
      if (weightingPercent != null && weightingPercent > 0) {
        return duration * ((double) weightingPercent / 100.0);
      }
    }
    return duration;
  };
