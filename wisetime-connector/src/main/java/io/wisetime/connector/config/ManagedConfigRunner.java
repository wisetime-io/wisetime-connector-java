...
public class ManagedConfigRunner extends TimerTask implements HealthIndicator {
  ...
  public ManagedConfigRunner(WiseTimeConnector wiseTimeConnector,
    ...
    this.lastSuccessfulRun = ZonedDateTime.now();
  ...
  public void run() {
    ...
    try {
      ...
      lastSuccessfulRun = ZonedDateTime.now();
  ...
  ZonedDateTime onManagedConfigResponse(ManagedConfigResponse configResponse, ZoneId zoneId) {
    ...
    OffsetDateTime serviceIdExpiry = configResponse.getServiceIdExpiry();
    return toExpiryZoneDateTime(serviceIdExpiry, zoneId);
  }

  @VisibleForTesting
  ZonedDateTime toExpiryZoneDateTime(OffsetDateTime serviceExpiry, ZoneId zoneId) {
    return serviceExpiry.toInstant().atZone(zoneId);
  }

  @Override
  public boolean isHealthy() {
    return ChronoUnit.MINUTES.between(lastSuccessfulRun, ZonedDateTime.now()) < maxMinsSinceSuccess;
  }
